...
import { Bell, RefreshCw, AlertTriangle, CheckCircle, Clock } from 'lucide-react';
...
export default function NotificationsAuditPage() {
  ...
  const fetchJobs = async () => {
    try {
      setLoading(true);
      const res = await systemService.getJobs('NOTIFICATION');
      if (res && res.data) {
        setJobs(res.data);
      }
    } catch (err) {
      toast.error('Failed to load notifications queue');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <header>
          <h1 style={{ fontSize: '1.5rem', fontWeight: '700', color: 'var(--text-primary)', marginBottom: '4px', display: 'flex', alignItems: 'center', gap: '8px' }}>
            <Bell size={24} color="var(--brand-primary)" /> Notifications Audit
          </h1>
          <p style={{ color: 'var(--text-secondary)' }}>Monitor all internal system notifications processed by the background worker.</p>
        </header>
        ...
        <table className="data-table" style={{ width: '100%', minWidth: '800px', fontSize: '13px' }}>
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={6} style={{ textAlign: 'center', padding: '32px', color: 'var(--text-tertiary)' }}>No notification jobs found.</td></tr>
            ) : (
              jobs.map(job => (
                <tr key={job.id}>
                  <td>
                  ...
                  <td style={{ fontWeight: '500' }}>{job.payload?.userId || '-'}</td>
                  <td>{job.payload?.title || '-'}</td>
                  <td>
                    <span className="badge" style={{ background: 'rgba(107, 70, 193, 0.1)', color: 'var(--primary)' }}>
                      {job.payload?.type || 'info'}
                    </span>
                  </td>
